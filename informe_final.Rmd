...
*   **Hipótesis de Varianza Contextual (Nivel 2):**
    *   **H1:** Existirá una varianza significativa en la confianza institucional que puede ser atribuida a diferencias sistemáticas entre los países (ICC > 0.05).
...
# --- TABLA DE MODELOS AL ESTILO DEL CURSO CON sjPlot ---
...
Para asegurar la robustez de los resultados, se realizó un análisis de influencia para detectar si algún país en particular estaba determinando los hallazgos. Como se observa en la Figura 3, ningún país supera el umbral crítico (línea roja), lo que indica que los resultados no están siendo indebidamente influenciados por un único caso. Esto confiere una alta confianza a la estabilidad de los coeficientes estimados y a las conclusiones del estudio.
...
names(distancias_cook) <- "cook.d" # Asignar un nombre de columna explícito
...
ggplot(distancias_cook, aes(x = reorder(nombrepais, cook.d), y = cook.d)) +
  ...
  labs(title = "Distancia de Cook por País", subtitle = "Mide la influencia de cada país en los coeficientes del modelo.", x = "País", y = "Distancia de Cook") +
