...
[Github repo](https://github.com/multinivel-facso/trabajo1-grupo-8/)
::::::
...
::::: columns
::: {.column width="50%"}
```{r grafico-interaccion-h5-combined, echo=FALSE, message=FALSE, warning=FALSE, fig.height=5, fig.width=5}
# --- PAQUETES Y MODELO ---
# Asegúrate de que 'm_final' (tu modelo con interacciones)
# ha sido creado en un chunk de setup anterior.
library(sjPlot)
library(ggplot2)
...
plot_model(
  m_final,
  type  = "int",
  terms = c("ind_desig_c", "gini_c") # VI Nivel 1, Moderador Nivel 2
) +
  labs(
    title  = "H5: Interacción AD x GINI",
    x      = "Aceptación de la Desigualdad (AD)",
    y      = "Confianza Institucional (Predicha)",
    colour = "Índice de GINI"
  ) +
  theme_minimal(base_size = 11) +
  theme(legend.position = "bottom") # Mueve la leyenda abajo para dar espacio
```
:::

::: {.column width="50%"}
```{r grafico-interaccion-h6-combined, echo=FALSE, message=FALSE, warning=FALSE, fig.height=5, fig.width=5}
# --- PAQUETES Y MODELO ---
# 'm_final' ya debería estar en el entorno desde el chunk anterior o setup.
library(sjPlot)
library(ggplot2)
...
plot_model(
  m_final,
  type  = "int",
  terms = c("ind_conf_priv_c", "gasto_social_c") # VI Nivel 1, Moderador Nivel 2
) +
  labs(
    title  = "H6: Interacción CSP x Gasto Social",
    x      = "Confianza en el Sector Privado (CSP)",
    y      = "Confianza Institucional (Predicha)",
    colour = "Gasto Social (% PIB)"
  ) +
  theme_minimal(base_size = 11) +
