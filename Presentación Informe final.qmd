...
:::
::: {.column width="40%"}
```{r boxplot-intro, fig.height=4.5}
ggplot(base_proc, aes(x = reorder(nombrepais, CIG, FUN = median), y = CIG)) +
   geom_boxplot(fill = "skyblue", alpha = 0.7) +
   coord_flip() +
  labs(
     title = "Confianza Institucional por País",
    x = "", y = "Índice de Confianza (CIG)"
   ) +
...
```{r tabla-modelos, results='asis'}
# Aquí iría el código para generar los modelos m0 a m4 y luego la tabla.
# m0 <- lmer(CIG ~ 1 + (1 | pais), data = base_proc)
# m1 <- lmer(CIG ~ CSP_cmc + AD_cmc + sexo + ... + (1 | pais), data = base_proc)
# ... etc.
# tab_model(m0, m1, m2, m3, m4, show.icc = TRUE, show.r2 = TRUE)
```
------------------------------------------------------------------------
