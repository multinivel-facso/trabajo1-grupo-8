...
```{r grafico-interaccion-final, echo=FALSE, warning=FALSE, message=FALSE, fig.align='center', fig-cap="Figura 1: Efecto de la Confianza en el Sector Privado sobre la Confianza Institucional, Moderado por la Desigualdad (GINI) del País."}
# --- CÓDIGO PARA EL GRÁFICO DE INTERACCIÓN ---

# 1. Cargar las librerías necesarias
pacman::p_load(ggeffects, ggplot2)

# 2. Calcular los efectos predichos para la interacción clave
# Le pedimos a ggeffects que calcule la confianza institucional predicha (y)
# a lo largo de los valores de confianza privada (x),
# para tres niveles distintos del moderador GINI (bajo, medio, alto).
# Se utiliza el modelo final, m3_interaccion.
predicciones_interaccion <- ggeffect(m3_interaccion, terms = c("ind_conf_priv_c", "gini_c"))

# 3. Crear el gráfico usando el objeto de predicciones y ggplot2 para personalizar
plot(predicciones_interaccion, connect.lines = TRUE) +
  # Etiquetas claras y descriptivas
  labs(
    title = "La Desigualdad Modera el Vínculo entre Confianza Privada e Institucional",
    subtitle = "El efecto positivo de la confianza en privados se debilita en países con mayor desigualdad.",
    x = "Confianza en el Sector Privado (Individual)",
    y = "Confianza Institucional (Predicha)",
    color = "Nivel de Desigualdad\n(Índice GINI)"
  ) +
  # Un tema visual limpio y profesional
  theme_minimal(base_size = 14) +
  # Usar una paleta de colores más atractiva y clara (opcional pero recomendado)
  scale_color_viridis_d(
    labels = c("GINI Bajo (-1 DE)", "GINI Promedio", "GINI Alto (+1 DE)")
  ) +
  theme(
